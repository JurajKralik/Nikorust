...
pub fn decide_units(bot: &mut Nikolaj) -> Vec<UnitTypeId> {
...
pub fn decide_next_structure(bot: &mut Nikolaj) -> Option<UnitTypeId> {
    let units: Vec<UnitTypeId> = bot.order_units.clone();
    //empty build order
    if units.len() == 0 {
        return None;
    }
...
pub(crate) fn execute_build_order(bot: &mut Nikolaj) {
    ...
    for unit in bot.order_units.clone() {
...
pub fn expand_production(bot: &mut Nikolaj) {
    if let Some(next_structure) = bot.next_multiproduction_structure.clone() {
